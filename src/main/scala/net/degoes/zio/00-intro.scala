...
object ZIOModel {
  ...
  final case class ZIO[-R, +E, +A](run: R => Either[E, A]) { self =>
    ...
    def flatMap[R1 <: R, E1 >: E, B](f: A => ZIO[R1, E1, B]): ZIO[R1, E1, B] =
      ZIO(r => map(f).run(r).flatMap(_.run(r)))
...
object PrintSequenceZip extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    putStrLn("Line1").zip(putStrLn("Line2")).zip(putStrLn("Line3")).exitCode
}
...
object PrintSequence extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    (putStrLn("Line1") *> putStrLn("Line2") *> putStrLn("Line3")).exitCode
}
...
object NumberGuesser extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] = for {
    randInt <- nextIntBounded(10)
    -       <- putStrLn("Guess the number between 0 and 9:")
    guess   <- readLine
...
object MultipleSideEffects extends App {
   ...
   * perform the print.
   */
  val getStrLn: Task[String] = ZIO.effect { readLine }
